	...
	public String execute() {
		...
		synchronized(session) {
			if ( session==null || session.getAttribute(Constants.SP_TOKEN)==null
				...
				Monitoring.monitoringLog( "<span class='error'>Step 2: Error!</span>");
		...
		if (session.getAttribute("cc") != null) return Action.SUCCESS;
		...
		Monitoring.monitoringLog( "<span class='success'>Step 2: Success!</span>");
		return Action.INPUT;
	}
